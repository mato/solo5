...
config_host_linux()
{
    ...
    # If we are cross-compiling, we don't currently build tools or tenders, so
    # stop here.
    [ -n "${CONFIG_CROSS_COMPILE}" ] && return
...
# Determine if we are cross-compiling. This needs to be done early, so is gated
# via an environment variable as opposed to an option.
if [ -n "${CROSS_COMPILE}" ]; then
    ...
    LD=${CROSS_COMPILE}-${LD:-ld}
    OBJCOPY=${CROSS_COMPILE}-${OBJCOPY:-objcopy}
else
...
if [ -n "${CONFIG_CROSS_COMPILE}" ]; then
    echo "${prog_NAME}: Cross-compiling, only bindings will be built."
fi
